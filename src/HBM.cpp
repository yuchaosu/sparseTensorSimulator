...
void HBMController::enableTrace(const std::string& path) {
	...
	if (!traceFp) {
		throw std::runtime_error("Failed to open HBM trace file");
	}
	std::fprintf(traceFp, "timestamp_ns,channel,evt,addr,size,requestId\n");
	std::fflush(traceFp);
	traceEnabled = true;
}
...
void HBMController::enableTraceAggregate(const std::string& path) {
	...
	if (!traceAggregateFp) {
		throw std::runtime_error("Failed to open HBM aggregate trace file");
	}
...
void HBMController::addRequest(uint64_t address, bool isWrite, uint64_t size, int requestId) {
	...
	// Trace enqueue event (timestamp uses controller currentTime)
	if (traceAggregateEnabled) {
		agg_enq_count++;
		agg_total_bytes += size;
		if (agg_first_ts == 0 || arrival < agg_first_ts) agg_first_ts = arrival;
		if (arrival > agg_last_ts) agg_last_ts = arrival;
	} else if (traceEnabled && traceFp) {
		std::fprintf(traceFp, "%llu,%d,ENQ,0x%llx,%llu,%d\n",
